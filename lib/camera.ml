...
let get_aspect {aspect_ratio; _} =
  aspect_ratio

let generate_ray {origin; target; vfov; aspect_ratio; vertical } x y = 
  let w = Vector.unit_vector (Vector.minus origin target) in
  let u = Vector.unit_vector (Vector.cross_prod vertical w) in
  let v = Vector.unit_vector (Vector.cross_prod u w) in

  let height = 2. *. Float.tan (vfov *. Float.pi /. 360.) in 
